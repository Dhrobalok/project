            ...
            <div class="card-header bg-white">
                <div class="row">
                    ...
                    <div class="col-md-6 text-right">
                        <a class="mr-2" href="{{ route('admin.voucher.transfer.view',['id' => $voucher -> id]) }}"
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-3 form-group">
                            <select class="form-control" id="transfer_method" name="transfer_method">
                                <option value="{{ $voucher -> transaction_method_id }}" selected hidden>
                                    {{ $voucher -> transaction_method_id == 1 ? 'Cash' : 'Bank' }}</option>
                                ...
                                <tr class="text-center">
                                    <td>
                                        <select class="form-control" name="transfer_from[]">
                                            ...
                                            @if($debit_accounts[$i] == $account -> id)
                                    ...
                                    <td>
                                        <select class="form-control" name="transfer_to[]">
                                            @foreach($accounts as $account)
                                            @if($credit_accounts[$i]  == $account -> id)
                                              <option value="{{ $account -> id }}" selected>{{ $account -> name }}</option>
                                            @else
                                            <option value="{{ $account -> id }}">{{ $account -> name }}</option>
                                            @endif
                                    ...
                                    <td>
                                        <input type="text" class="form-control" name="descriptions[]" required value = "{{ $descriptions[$i] }}">
                                    </td>
                                    <td>
                                        <input type="number" step=".000001" class="form-control" name="amounts[]" value = "{{ $amounts[$i] }}">
                                    </td>
                                    <td>
...
$(document).ready(function() {
    ...
    $('#save_transfer_voucher').submit(function(e) {
        ...
        if (confirm('Are you sure to post that voucher?')) {
            ...
            $.ajax({
                url: "{{ route('admin.voucher.transfer.update',['id' => $voucher -> id]) }}",
                type: "post",
                data: voucherData,
                processData: false,
                contentType: false,
