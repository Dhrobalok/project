...
<div class="block block-rounded" style="margin-left:22%;margin-right:2%;margin-top:7%;">
    <div class="block-header block-header-default">
        <h3 class="block-title">Transfer Vouchers</h3>
        @can('create vouchers')
        <a href = "{{ route('admin.voucher.transfer.create') }}"><i class = "fa fa-plus-circle mr-2"></i>Add New</a>
        @endcan
    </div>
            ...
            <tbody>
                ...
                <tr id = "{{ $voucher -> id }}" class = "text-center">
                    ...
                    <td>
                        <div class="btn-group">
                            @can('view vouchers')
                            <a class="btn btn-info mr-1" href="{{ route('admin.voucher.transfer.view',['id' => $voucher -> id]) }}"
                                ><i
 ...
 function delete_voucher(id)
 {
    Swal.fire({
      ...
      if (result.isConfirmed) {
          ...
          $.ajax({
              url:"{{ route('admin.voucher.transfer.delete') }}",
              type:"POST",
              data:
              {
                 id : id
